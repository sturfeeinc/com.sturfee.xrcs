        ...
        public Texture2D TakeObjectSnapshot(GameObject prefab, GameObject fallback, Vector3 position, Quaternion rotation, Vector3 scale)
        {
            ...
            if (DestroyScripts)
            {
                ...
                for (int i = 0; i < scripts.Length; ++i)
                {
                    ...
                    //if (scripts[i].GetType() == typeof(TriLibCore.AssetUnloader))
                    //{
                    //    MyLogger.Log($"Skip removing script 'TriLibCore.AssetUnloader'");
                    //    continue;
                    //}

                    if (scripts[i].GetType().FullName.StartsWith("TriLibCore"))
                    {
                        MyLogger.Log($"Skip removing script starting with 'TriLibCore'");
                        continue;
                    }

                    //DestroyImmediate(scripts[i]);

                    if (scripts[i].CanDestroy(scripts[i].GetType()))
                    {
                        DestroyImmediate(scripts[i]);
                    }
                }
